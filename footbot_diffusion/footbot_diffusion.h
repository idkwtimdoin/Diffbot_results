...
private:
   ...
   CRange<CRadians> m_cGoStraightAngleRange;    // never used

   /*
    * Returns a coefficient to be multiplied by the wheel velocity
    * according to the accumulated obstacle angle
    */
   Real DefusionDelta(CRadians angle);

   /* Simulates the exponential nature of the proximity sensor
    * Takes the sensor's [0:1] reading
    * Returns an exponentially scaled reading
   */
   Real ExpoScale(Real reading);
